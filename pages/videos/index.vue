    ...
    <section class="margin-section padding-section">
        <div class="container shadow py-3"><!-- Start: search category -->
            <div class="pb-4"><!-- Start: item -->
                <div class="d-inline-flex unselect-category" :class="{' select-category':selectTags?.includes(tags.label)}" v-for="(tags,index) in tagss" v-bind:key="tags._id" @click="div2963a3133d6b77171e11f88604c59be6178dc1ddclick(tags,index)"><span>#</span>
                    <p>{{tags.label}}</p>
                </div><!-- End: item -->
...
;
    ...
    const div2963a3133d6b77171e11f88604c59be6178dc1ddclick = async function(tags,index){
        if(!selectTags.value.includes(tags.value?.label)){selectTags.value.push(tags.value?.label)}else{selectTags.value = selectTags.value.filter(item => item !== tags.value?.label)};setQuery('tag',selectTags.value);await loadDatavideos();
    };
...
webConfig.value = {...resultwebConfig.data.value?.webConfig}}
  ...
  // watch
    ...
    title =''
         ...
         title= 'Video Hair Transplant';
            meta.push({ name: 'title', content:'Video Hair Transplant' });
            
            meta.push({ name: 'og:image', content:webConfig?.value.mediaSeo });
            
            meta.push({ name: 'description', content:'Watch our Video Hair Transplant guide to see the procedure in action and understand the step-by-step process. Learn how advanced techniques deliver natural, lasting results. Get informed and confident about your hair restoration journey…' });
            
            meta.push({ name: 'keywords', content: 'Video Hair Transplant'});
...
//update in Thu Nov 28 2024 03:04:44 GMT+0000 (Coordinated Universal Time) by loidirieng
